    ...
    sizes.forEach(size => {
        describe(`${size} entries`, () => {
            ...
            beforeAll(() => {
                ...
                console.log(`[${size}] MiniSearch Indexing: ${(end - start).toFixed(2)}ms`);
            ...
            test('MiniSearch: Search time (prefix)', () => {
                ...
                const end = performance.now();
                console.log(`[${size}] MiniSearch Search (prefix): ${(end - start).toFixed(2)}ms, found ${results.length}`);
            ...
            test('MiniSearch: Search time (fuzzy)', () => {
                ...
                const end = performance.now();
                console.log(`[${size}] MiniSearch Search (fuzzy): ${(end - start).toFixed(2)}ms, found ${results.length}`);
            ...
            test('Naive Filter: Search time', () => {
                ...
                const end = performance.now();
                console.log(`[${size}] Naive Filter Search: ${(end - start).toFixed(2)}ms, found ${results.length}`);
            });
        });
    });
});
