...
export class IntrospectionService {
    ...
    public getTemplateVariables(library: Library): VariableDefinition[] {
        ...
        for (const [key, description] of Object.entries(KNOWN_VARIABLE_DESCRIPTIONS)) {
        ...
        for (const entry of entries) {
            ...
            for (const [key, value] of Object.entries(entryJson)) {
                if (key.startsWith('_') || typeof value === 'function' || typeof value === 'object') {
        ...
        return Array.from(variables.values()).sort((a, b) => a.key.localeCompare(b.key));
    }
}
